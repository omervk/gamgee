...
export class WorkflowWorker {
    ...
    async executeWaitingTask(store: StateStore, query: Query, fetchTimeoutMs: number): Promise<WorkflowTask | "Workflow Completed" | "No Tasks Waiting"> {
    ...
    async executeWaitingWorkflow(store: StateStore, query: Query, fetchTimeoutMs: number): Promise<"Workflow Completed" | "No Tasks Waiting"> {
        const maybeTask = await store.tryFetchingTask(query, fetchTimeoutMs);
