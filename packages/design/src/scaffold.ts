...
export abstract class ${workflowTypeName}Base extends WorkflowBase {
    protected constructor() {
        super('${workflowTypeName}');
...
function workflowTitleToTypeName(titleName: string) {
    return pascalCase(titleName).replace(/(Workflow)?$/i, 'Workflow')
}

function typeFromDecision(workflowTypeName: string, node: ChoiceNode) {
    const options = Object.keys(node.goesTo)
        .map(name => `\n    | Promise<ReturnType<${workflowTypeName}Base['${node.name}']['${camelCase(name)}']>>`)
        .join('')

    return `export type ${pascalCase(node.name)}Options =${options}`
}
...
`
        ...
        returnType = `${pascalCase(nextStep.name)}Options`
    }
...
export function mermaidToScaffold(
    ...
    const workflowTypeName = workflowTitleToTypeName(titleName)
    ...
    const decisionTypes: string[] = Object.values(choices).map(d => typeFromDecision(workflowTypeName, d))
